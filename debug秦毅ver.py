...
class Player():
	...
	#印東西用的global fcn
	def printText(presentPlayer):#傳入Player物件
		for textline in presentPlayer.showText:
			text = font.render(each, True, white, textColorInMessageBox)
			screen.blit(text,textPosition)#印上去		  
			textPosition[1] += 30#換行
...
def main():
    ...
    pygame.display.set_caption("誰能先畢業? 台大大富翁")
   ...
   # 循環開始！ 
    while running:
        ...
        if selectCharacter:				 
            ...
            #選角介面設定
            screen.blit(character_selection , (0,0))#不是很確定selectCharacter?       
	    ...
	    #在每一個地點上，印出血量
                        ...
                        if presentPlayer.movable:#有時候會被設為False
							...
							#主動技能
                                    ...
                                    if pressYes == True:
                                        ...
                                        textline4 = '哈哈哈'
                                        presentPlayer.showText = [textline3, textline4])
                                        printText(presentPlayer)
								...
								textline6 = '擲骰子點數翻倍!'
								presentPlayer.showText = [textline5, textline6]
								printText(presentPlayer)
							...
							presentPlayer.showText = ["你這一回合不能動!"]
                            printText(presentPlayer)
                            
							
				#按回合結束鍵，更新presentPlayer是誰     
                if turnover_rect.collidepoint(event.pos): 
					if presentPlayer.movable == False:#下一回合可以動了
						presentPlayer.movable = True                    
					showButton2 = True 
                    playerCount += 1
                    if playerCount == 4:
                        playerCount -= 4  
            ...
            for each in allplayers:
				...
				if each.money < 0:
					loseText = font.render(each.name +'is loser 沒錢還想讀書!!', True, red)
