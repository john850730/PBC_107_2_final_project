...
characters_dict = \
   {'土木': {'credit': 140, 'attack': 4, 'ActiveAbility': None, 'PassiveAbility': '收回扣', 'definition': '買完土地、或建蓋城堡後，可以回收10%消耗的錢幣'}, \
    '機械': {'credit': 140, 'attack': 4, 'ActiveAbility': '工具人', 'PassiveAbility': '無', 'definition': '蓋城堡時可直接升級城堡血量10點'}, \
    '國企': {'credit': 129, 'attack': 4, 'ActiveAbility': 'elite光環', 'PassiveAbility': '無', 'definition': '遊戲介面上的角色發出光芒一下下'}, \
   ...
   # 循環開始！ 
    while running:
        ...
        if gameStarted and allset == 4: # 角色選好後，會變成true，遊戲開始
            ...
            # 錢幣、學分公佈欄
            board = font.render('*錢幣 & 學分數* 排行榜' , True, black, white)
            ...
            # 攻擊力、技能P公佈欄
            P_1 = font.render('%s的攻擊力為%d, 技能為【%s】' % (player1.name, player1.attack, player1.PassiveAbility), True, black, white)
            P_2 = font.render('%s的攻擊力為%d, 技能為【%s】' % (player2.name, player2.attack, player2.PassiveAbility), True, black, white)
            P_3 = font.render('%s的攻擊力為%d, 技能為【%s】' % (player3.name, player3.attack, player3.PassiveAbility), True, black, white)
            P_4 = font.render('%s的攻擊力為%d, 技能為【%s】' % (player4.name, player4.attack, player4.PassiveAbility), True, black, white)
            
            screen.blit(P_1, (280,245))
            screen.blit(P_2, (280,275))
            screen.blit(P_3, (280,305))
            screen.blit(P_4, (280,335))
